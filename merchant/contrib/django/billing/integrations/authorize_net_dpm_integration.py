...
from merchant.integrations.authorize_net_dpm_integration import AuthorizeNetDpmIntegration as Integration
from merchant.contrib.django.billing.forms.authorize_net_forms import AuthorizeNetDPMForm
...
class AuthorizeNetDpmIntegration(Integration):
    ...
    def generate_form(self):
        transaction_key = self.settings["TRANSACTION_KEY"]
    ...
    def verify_response(self, request):
        ...
        md5_hash = self.settings["MD5_HASH"]
        login_id = self.settings["LOGIN_ID"]
    ...
    def get_urls(self):
        urlpatterns = patterns('',
           url('^merchant/authorize_net-notify-handler/$', self.authorizenet_notify_handler, name="merchant_authorize_net_notify_handler"),
           url('^merchant/authorize_net-sucess-handler/$', self.authorize_net_success_handler, name="merchant_authorize_net_success_handler"),
           url('^merchant/authorize_net-failure-handler/$', self.authorize_net_failure_handler, name="merchant_authorize_net_failure_handler"),)
        return urlpatterns
